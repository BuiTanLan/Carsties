        ...
        IEventService events) : PageModel
{
    ...
    public async Task OnGet()
    {
        ...
        foreach (var grant in grants)
        {
            ...
            if (client != null)
            {
                var listResources = await resources.FindResourcesByScopeAsync(grant.Scopes);
                ...
                var item = new GrantViewModel()
                {
                    ...
                    IdentityGrantNames = listResources.IdentityResources.Select(x => x.DisplayName ?? x.Name).ToArray(),
                    ApiGrantNames = listResources.ApiScopes.Select(x => x.DisplayName ?? x.Name).ToArray()
                };
